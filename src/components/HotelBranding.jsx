...
export default function HotelBranding({ hotel }) {
  return (
    <section className="mx-auto max-w-7xl px-4 pb-8 sm:px-6 sm:pb-10 lg:px-8">
      <div className="overflow-hidden rounded-[32px] border border-white/85 bg-[#f7eede]/80 p-6 shadow-[0_18px_55px_rgba(94,26,42,0.09)] sm:p-8 lg:p-10">
        <div className="grid gap-7 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
          <div className="flex min-w-0 flex-col gap-5 sm:flex-row sm:items-start">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-3 max-w-2xl text-sm leading-7 text-[#7c6a5e] sm:text-[15px]">
...
function TrustItem({ icon, title, text }) {
  return (
    <div className="flex items-start gap-3 rounded-[24px] border border-white/85 bg-white/68 p-4 shadow-sm">
      ...
      <div>
        <h3 className="text-sm font-semibold text-[#2a1d18]">{title}</h3>
        <p className="mt-1 text-xs leading-5 text-[#9a877a]">{text}</p>
      </div>
    </div>
  );
}
